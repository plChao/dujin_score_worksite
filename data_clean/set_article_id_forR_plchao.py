...
output_csv = '../2024_table/actual_exam_situation.csv'
artical_df_path = '../2024_table/article_info.csv'
import os
...
if os.path.exists(artical_df_path):
    previous_df = pd.read_csv(artical_df_path)
    artical_df = pd.concat([previous_df, artical_df])
artical_df = artical_df.drop_duplicates()
artical_df.sort_values(by='article_id', inplace=True, ignore_index=True)
artical_df.to_csv(artical_df_path, index=False)